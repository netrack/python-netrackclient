...
class NetworkModuleManager(object):

    def __init__(self, client):
        super(NetworkModuleManager, self).__init__()

        self.client = client

    def _url(self, datapath, module):
        url = "{url_prefix}/datapaths/{datapath}/network/mechanisms".format(
                datapath=datapath,
    ...
    def _check_module(self, module):
        if not module:
            raise errors.NetworkError({"error": "not a network module"})
